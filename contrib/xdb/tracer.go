...
func GetSpanFromContext(ctx context.Context, proto, sql, operation string, stackSkip int) (nctx context.Context, span trace.Span) {
	...
	ctx, span = tracer.Start(ctx, fmt.Sprintf("XDB:%s", operation),
		...
		trace.WithAttributes(
			attribute.String("db.system", proto),
