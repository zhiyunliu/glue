...
## CRON 定时任务服务
    使用了时间轮算法对任务进行任务派发，通过cron 表达式来计算任务的执行。同时支持多程序主备功能

 
# 架构图

![avatar](./images/架构图.png "架构图")
...
├── config 配置中心配置 URI格式
├── caches 缓存配置
│   └── 项数组{"proto"：使用的协议,"addr":配置的类型和名称,URI 格式配置},
├── queues 消息队列配置
│   └── 队列数组{"proto"：使用的协议,"addr":配置的类型和名称,URI 格式配置},
├── dbs 数据库配置
│   └── 队列字典"数据库名称":{"proto":"使用的协议","conn":"链接串","max_open":10,"max_idle":10,"life_time":100},
├── servers 服务器列表
│   └── apiserver 服务名称,对应 api.New("服务名称")
│   ├────└──服务配置,各种服务有些差异，具体请看对眼server/服务类型/config.go 文件
